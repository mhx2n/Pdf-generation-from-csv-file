...
def _start_health_server() -> None:
    """Tiny HTTP server so Render Free Web Service detects an open PORT.
    Render kills services that don't bind to $PORT within ~60s."""
    import os, threading
    ...
    class H(BaseHTTPRequestHandler):
        ...
        def log_message(self, *a, **k):  # silence access logs
            return
...
def main() -> None:
    init_db()
    _start_health_server()  # MUST be before run_polling for Render Web Service
